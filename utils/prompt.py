...
4. 驾车规划：用户询问驾车路线。
5. 公交规划：用户询问公交路线。
6. 地点推荐：用户询问某地附近的POI，包括景点、酒店、餐馆、商场、医院等。
7. 图片生成：用户需要生成图片。
8. 图片理解：用户需要你识别图片内容后进行回答。
9. 视频生成：用户需要生成视频。
10. 数学解题：用户需要调用数学解题能力，只能是数学题。
'''
...
1.意图识别：接入高德API，可根据你提供的地理信息查询：天气，路线，景点/医院/美食等；
2.百科问答：接入大模型API，回答你的一切疑问，遇事不决，就问小爱。
3.图像生成：接入FLUX API，根据输入的提示词，给你返回一张图像。
4.视频生成：本地部署CogVideo开源模型，根据输入提示词返回3s视频。
5.图像理解：接入多模态模型，基于输入图像，回答你的问题。
6.拍照解题：本地部署OCR+千问数学大模型，支持图片输入和文本输入。
7.多轮对话：基于本地数据库实现多轮对话和记忆。
8.搜索增强：添加触发词‘搜索’，小爱会调用搜索引擎解决你的问题。
9.公众号接入：添加触发词‘猴哥’，小爱会调用#公众号：猴哥的AI知识库，基于历史发表文章回答你的问题。
要求：
...
【猴哥团队能提供】
1. AI模型私有化部署，模型训练，模型微调
2. 微信AI机器人搭建-定制化服务
3. Coze/Dify等智能体搭建

如需体验【小爱】微信AI机器人，可回复【加群】，进群请看群公告
'''